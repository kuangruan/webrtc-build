...
def get_build_targets(target):
    ts = [":default"]
    if target not in ("windows_x86_64", "windows_arm64", "ios", "macos_arm64","macos_x86_64"):
...
def build_webrtc(
    ...
    if not os.path.exists(os.path.join(webrtc_build_dir, "args.gn")) or gen:
        ...
        elif target in ("macos_arm64","macos_x86_64"):
            gn_args += [
                'target_os="mac"',
    ...
    if (target in ("macos_arm64","macos_x86_64")) and not nobuild_macos_framework:
        branch, commit, revision, maint = get_webrtc_version_info(version_info)
        info = {}
        info["branch"] = branch
        info["commit"] = commit
...
def package_webrtc(
    ...
    if target in ["ios", "macos_arm64","macos_x86_64"]:
...
def check_target(target):
    ...
    if platform.system() == "Windows":
        ...
        logging.info(f"OS: {platform.system()}")
        return target in ("macos_arm64", "ios","macos_x86_64")
